...
- Destructive actions are one-second holds (Raise+W clears Bluetooth, Raise+D enters the bootloader), never combos: a Base combo's one-shot Raise makes a repeated chord a Raise combo. Both keyboards run this firmware; the older X+G bootloader combo is gone.
